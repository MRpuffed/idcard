...
class ApplicationUI(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.master.title('身份证号码生成器')
        self.master.geometry('495x450+468+165')
        self.master.resizable(width=False, height=False)

        # 创建控件
        self.style = Style()
        self.frm_auto = LabelFrame(self.top, text='随机生成', style='frm_auto.TLabelframe')
        self.lbl_frm_auto_txt1 = Label(self.frm_auto, text='随机生成', style='lbl_frm_auto_txt1.TLabel')
        self.ety_frm_auto_count = Entry(self.frm_auto, text='1', font=('微软雅黑', 9))
        self.lbl_frm_auto_txt2 = Label(self.frm_auto, text='个身份证号码', style='lbl_frm_auto_txt2.TLabel')
        self.txt_frm_auto_id = Text(self.frm_auto, wrap=WORD, font=('微软雅黑', 9))
        self.btn_frm_auto_generate = Button(self.frm_auto, text='开始生成', style='btn_frm_auto_generate.TButton')

        self.frm_condition = LabelFrame(self.top, text='选择生成', style='frm_condition.TLabelframe', relief='groove',
                                        borderwidth="4")
        self.lbl_frm_condition_district = Label(self.frm_condition, text='选择地区：',
                                                style='lbl_frm_condition_district.TLabel')
        self.cmb_frm_condition_province = Combobox(self.frm_condition, font=('微软雅黑', 9))
        self.cmb_frm_condition_city = Combobox(self.frm_condition, font=('微软雅黑', 9))
        self.cmb_frm_condition_district = Combobox(self.frm_condition, font=('微软雅黑', 9))
        self.cmb_frm_condition_year = Combobox(self.frm_condition, font=('微软雅黑', 9))
        self.cmb_frm_condition_month = Combobox(self.frm_condition, font=('微软雅黑', 9))
        self.cmb_frm_condition_day = Combobox(self.frm_condition, font=('微软雅黑', 9))
        self.lbl_frm_condition_birthday = Label(self.frm_condition, text='出生日期：',
                                                style='lbl_frm_condition_birthday.TLabel')
        self.lbl_frm_condition_gender = Label(self.frm_condition, text='性别：', style='lbl_frm_condition_gender.TLabel')
        self.cmb_frm_condition_gender = Combobox(self.frm_condition, font=('微软雅黑', 9))
        self.lbl_frm_condition_id = Label(self.frm_condition, text='身份证号码：', style='lbl_frm_condition_id.TLabel')
        self.ety_frm_condition_id = Entry(self.frm_condition, font=('微软雅黑', 9))
        self.btn_frm_condition_generate = Button(self.frm_condition, text='开始生成',
                                                 style='btn_frm_condition_generate.TButton')

        self.frm_parse = LabelFrame(self.top, text='解析身份证号码', style='frm_parse.TLabelframe')
        self.label_frm_parse_id = Label(self.frm_parse, text='身份证号码：', style='label_frm_parse_id.TLabel')
        self.ety_frm_parse_id = Entry(self.frm_parse, font=('微软雅黑', 9))
        self.lbl_frm_parse_district = Label(self.frm_parse, text='地区：', style='lbl_frm_parse_district.TLabel')
        self.ety_frm_parse_district = Entry(self.frm_parse, text='广东省深证市南山区', font=('微软雅黑', 9))
        self.lbl_frm_parse_birthday = Label(self.frm_parse, text='生日：', style='lbl_frm_parse_birthday.TLabel')
        self.ety_frm_parse_birthday = Entry(self.frm_parse, font=('微软雅黑', 9))
        self.ety_frm_parse_gender = Entry(self.frm_parse, font=('微软雅黑', 9))
        self.lbl_frm_parse_gender = Label(self.frm_parse, text='性别：', style='lbl_frm_parse_gender.TLabel')
        self.btn_frm_parse_reset = Button(self.frm_parse, text='清空内容', style='btn_frm_parse_reset.TButton')
        self.btn_frm_parse_parse = Button(self.frm_parse, text='开始解析', style='btn_frm_parse_parse.TButton')

        self.create_widgets()

    def create_widgets(self):
        # 随机生成身份证号码
        self.style.configure('frm_auto.TLabelframe', font=('微软雅黑', 9))
        self.frm_auto.place(x=5, y=5, width=190, height=240)

        self.style.configure('lbl_frm_auto_txt1.TLabel', anchor='center', font=('微软雅黑', 9))
        self.lbl_frm_auto_txt1.place(x=0, y=10, width=70, height=23)

        self.ety_frm_auto_count.place(x=70, y=10, width=30, height=23)

        self.style.configure('lbl_frm_auto_txt2.TLabel', anchor='w', font=('微软雅黑', 9))
        self.lbl_frm_auto_txt2.place(x=100, y=10, width=80, height=23)

        self.txt_frm_auto_id.place(x=20, y=50, width=140, height=105)

        self.style.configure('btn_frm_auto_generate.TButton', font=('微软雅黑', 9))
        self.btn_frm_auto_generate.place(x=20, y=170, width=70, height=25)

        # 选择生成身份证号码
        self.style.configure('frm_condition.TLabelframe', font=('微软雅黑', 9))
        self.frm_condition.place(x=190, y=5, width=300, height=240)

        self.style.configure('lbl_frm_condition_district.TLabel', anchor='w', font=('微软雅黑', 9))
        ...
        self.style.configure('lbl_frm_condition_birthday.TLabel', anchor='w', font=('微软雅黑', 9))
        self.lbl_frm_condition_birthday.place(x=10, y=50, width=60, height=23)

        self.cmb_frm_condition_year.place(x=80, y=50, width=60, height=23)

        self.cmb_frm_condition_month.place(x=140, y=50, width=50, height=23)

        self.cmb_frm_condition_day.place(x=190, y=50, width=60, height=23)

        self.style.configure('lbl_frm_condition_gender.TLabel', anchor='w', font=('微软雅黑', 9))
        self.lbl_frm_condition_gender.place(x=10, y=90, width=50, height=23)

        self.cmb_frm_condition_gender.place(x=80, y=90, width=40, height=23)

        self.style.configure('lbl_frm_condition_id.TLabel', anchor='w', font=('微软雅黑', 9))
        ...
        self.style.configure('btn_frm_condition_generate.TButton', font=('微软雅黑', 9))
        self.btn_frm_condition_generate.place(x=10, y=170, width=70, height=25)

        # 解析身份证号码
        self.style.configure('frm_parse.TLabelframe', font=('微软雅黑', 9))
        self.frm_parse.place(x=5, y=241, height=200, width=485)

        self.style.configure('label_frm_parse_id.TLabel', anchor='w', font=('微软雅黑', 9))
        self.label_frm_parse_id.place(x=20, y=20, width=80, height=23)

        self.ety_frm_parse_id.place(x=100, y=20, width=140, height=23)

        self.style.configure('lbl_frm_parse_district.TLabel', anchor='w', font=('微软雅黑', 9))
        ...
        self.style.configure('lbl_frm_parse_birthday.TLabel', anchor='w', font=('微软雅黑', 9))
        ...
        self.style.configure('lbl_frm_parse_gender.TLabel', anchor='w', font=('微软雅黑', 9))
        ...
        self.style.configure('btn_frm_parse_parse.TButton', font=('微软雅黑', 9))
        self.btn_frm_parse_parse.place(x=20, y=145, width=70, height=25)

        self.style.configure('btn_frm_parse_reset.TButton', font=('微软雅黑', 9))
        self.btn_frm_parse_reset.place(x=180, y=145, width=70, height=25)
...
class Application(ApplicationUI):
    ...
    def provinces(self):
        all_provinces = []
        for i in range(len(list_prvoince)):
    ...
    def cities(self):
        ...
        for i in range(len(list_prvoince)):
            if list_prvoince[i]['name'] == self.cmb_frm_condition_province.get():
                current_province_code = list_prvoince[i]['code']

        current_cities = []
        for x in range(len(list_city)):
            if list_city[x]['code'][0:2] == current_province_code[0:2]:
                current_cities.append(list_city[x]['name'])
    ...
    def districts(self):
        ...
        for j in range(len(list_city)):
            if list_city[j]['name'] == current_city and list_city[j]['code'][0:2] == current_province_code[0:2]:
                current_city_code = list_city[j]['code']
        print(current_city_code)
        current_districts = []

        for x in range(len(list_district)):
            if list_district[x]['code'][0:4] == current_city_code[0:4]:
                current_districts.append(list_district[x]['name'])
